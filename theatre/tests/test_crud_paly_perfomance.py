...
class PlayTests(TestCase):
    ...
    def test_retrieve_play(self):
        play = Play.objects.create(
            title="Romeo and Juliet", description="Classic love story"
        )
    ...
    def test_update_play(self):
        play = Play.objects.create(title="Macbeth", description="Original description")
        payload = {"description": "Updated description"}

        response = self.client.patch(f"/api/theatre/plays/{play.id}/", payload)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            Play.objects.get(id=play.id).description, "Updated description"
        )

    def test_delete_play(self):
        play = Play.objects.create(title="Hamlet", description="Test description")
...
class PerformanceTests(TestCase):
    ...
    def test_create_performance(self):
        response = self.client.post(
            "/api/theatre/performances/", self.base_performance_data
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_retrieve_performance(self):
        ...
        response = self.client.get(f"/api/theatre/performances/{performance.id}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_performance(self):
        ...
        response = self.client.put(
            f"/api/theatre/performances/{performance.id}/", updated_data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["show_time"], "2024-03-11T20:00:00Z")
    ...
    def test_delete_performance(self):
        ...
        response = self.client.delete(f"/api/theatre/performances/{performance.id}/")
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
